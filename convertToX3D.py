...
if platform.system() == 'Windows': 
    FREECADPATH = glob.glob(r"D:\ProgrammiUnsupported\FreeCAD *\bin")
...
def converter(filesPath, totalFiles, file):
    ...
    if not os.path.exists(filesPath + "/Converted-X3Ds"):
        os.makedirs(filesPath + "/Converted-X3Ds")

    if not os.path.exists(filesPath + "/OriginalFiles"):
        os.makedirs(filesPath + "/OriginalFiles")

    newName = filesPath + "/Converted-X3Ds/" + file + ".xhtml"
...
def main():
    ...
    print("Files to convert: " + str(totalFiles))
